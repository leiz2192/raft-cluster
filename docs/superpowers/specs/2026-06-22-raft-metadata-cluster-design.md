...
### 2.1 三节点拓扑（同端口、不同回环地址）

三节点用**相同端口**、靠 127.0.0.0/8 内不同回环地址区分（整段 127.x.x.x 均为 loopback，Linux 上可分别 bind，故同端口不冲突）：

| 节点 | Raft 地址 (TCP) | HTTP 地址 | 调试 pprof | 数据目录 |
|------|----------------|----------|-----------|----------|
...
三个节点是**对等**的同一个二进制 `raft-meta`，靠配置文件区分身份（节点 id、回环地址、数据目录、集群 peer 列表）。端口跨节点一致，便于记忆与脚本统一。
